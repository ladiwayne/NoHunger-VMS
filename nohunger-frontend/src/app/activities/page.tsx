  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        const appliedIds: string[] = volunteer?.appliedActivities?.map((a: any) => a._id || a.id) || [];
        appliedIds.forEach((id) => {
          appMap[id] = { activity_id: id, status: 'pending' };
        });
        visible.forEach((a) => {
          const approved = (a as any).volunteersApproved?.some((v: any) => (v._id || v.id) === user.id);
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {[...Array(6)].map((_, i) => <div key={i} className="h-64 bg-card border border-border rounded-2xl animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {filtered.map(activity => {
              ...
              return (
                <div key={activity.id} className="bg-card border border-border rounded-2xl shadow-card hover:shadow-[0_4px_16px_0_rgba(22,101,52,0.12)] hover:border-[hsl(142,72%,78%)] transition-all flex flex-col">
                  <div className="p-5 flex-1">
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <span className={`text-[11px] font-700 px-2.5 py-1 rounded-full border uppercase tracking-wide ${getStatusColor(activity.status)}`}>
                        {activity.status}
                      </span>
                      {activity.activity_type && (
                        <span className="text-[11px] text-muted-foreground font-500 capitalize">{(activity.activity_type || '').replace('_', ' ')}</span>
                      )}
                    </div>
                    <h3 className="text-[15px] font-700 text-foreground mb-2 leading-snug">{activity.title}</h3>
                    {activity.description && (
                      <p className="text-[13px] text-muted-foreground leading-relaxed mb-3 line-clamp-2">{activity.description}</p>
                    )}
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2 text-[12.5px] text-muted-foreground">
                        <Calendar size={13} className="flex-shrink-0" />
                        <span>{new Date(activity.start_date || activity.startDate).toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      </div>
                      {activity.location && (
                        <div className="flex items-center gap-2 text-[12.5px] text-muted-foreground">
                  ...
                  <div className="px-5 pb-5">
                    {app ? (
                      <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-600 ${
                        app.status === 'approved' ? 'bg-success/10 text-success' :
                        app.status === 'rejected'? 'bg-destructive/10 text-destructive' : 'bg-warning/10 text-warning'
                      }`}>
                        <CheckCircle2 size={14} />
                        Application {app.status}
                      </div>
                    ) : isPast ? (
                      <div className="px-4 py-2.5 bg-muted rounded-xl text-[13px] font-600 text-muted-foreground text-center">Event ended</div>
                    ) : (
