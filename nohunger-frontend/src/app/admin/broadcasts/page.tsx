...
export default function AdminBroadcastsPage() {
  ...
  const [form, setForm] = useState({
    title: '', message: '', target_type: 'all', target_activity_id: '', target_group_id: '',
  });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (form.target_type === 'activity' && form.target_activity_id) {
        ...
        const ids = (activity?.volunteersApproved || []).map((v: any) => v._id || v.id).filter(Boolean);
        if (ids.length === 0) {
      ...
      setForm({ title: '', message: '', target_type: 'all', target_activity_id: '', target_group_id: '' });
      fetchData();
    } catch (err: any) {
    ...
    <AppLayout activePath="/admin/broadcasts">
      <div className="space-y-6 animate-fade-in">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {form.target_type === 'group' && (
                <div>
                  <label className="block text-[13px] font-600 text-foreground mb-1.5">Select Group</label>
                  <select
        ...
        <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
          ...
          {loading ? (
            <div className="p-5 space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-muted rounded-xl animate-pulse" />)}</div>
          ) : broadcasts.length === 0 ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-[11px] text-muted-foreground">
                            {new Date(b.created_at || b.sent_at).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </span>
