...
export default function HoursTrackingPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setMonthlyData(Object.entries(months).map(([month, hours]) => ({ month, hours: Math.round(hours * 10) / 10 })));
    } catch (err) {
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-[14px] text-muted-foreground mt-0.5">Your complete volunteer hours log</p>
          </div>
          <button
            onClick={exportCsv}
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-card border border-border rounded-2xl animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Hours', value: stats.total, unit: 'hrs', icon: Clock, color: 'text-primary', bg: 'bg-primary/10' },
              { label: 'This Month', value: stats.thisMonth, unit: 'hrs', icon: CalendarCheck, color: 'text-success', bg: 'bg-success/10' },
              { label: 'Events Attended', value: stats.events, unit: 'events', icon: CheckCircle2, color: 'text-primary', bg: 'bg-primary/10' },
              { label: 'Avg per Event', value: stats.avgPerEvent, unit: 'hrs', icon: Clock, color: 'text-muted-foreground', bg: 'bg-muted' },
            ].map(card => {
              const Icon = card.icon;
              return (
                <div key={card.label} className="bg-card border border-border rounded-2xl p-5 shadow-card">
                  <div className={`w-9 h-9 rounded-xl ${card.bg} flex items-center justify-center mb-3`}>
                    <Icon size={18} className={card.color} />
                  </div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-700 font-tabular text-foreground">{card.value}</span>
                    <span className="text-[12px] text-muted-foreground">{card.unit}</span>
                  </div>
                  <p className="text-[12px] font-600 text-muted-foreground mt-0.5 uppercase tracking-wide">{card.label}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-card p-5">
          ...
          <p className="text-[12px] text-muted-foreground mb-5">Hours volunteered over the last 6 months</p>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
            <div className="p-5 border-b border-border flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground">{filtered.length} completed sessions</p>
              </div>
            </div>
            {loading ? (
              <div className="p-5 space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-12 bg-muted rounded-xl animate-pulse" />)}</div>
            ) : filtered.length === 0 ? (
              <div className="p-10 text-center">
                ...
                <p className="text-[12px] text-muted-foreground mt-1">Your completed volunteer sessions will appear here</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {filtered.map((s, i) => (
                      <tr key={s.id} className={`border-b border-border last:border-0 hover:bg-muted/30 transition-colors ${i % 2 === 0 ? '' : 'bg-muted/20'}`}>
                        <td className="px-4 py-3 text-[13px] font-600 text-foreground max-w-[200px] truncate">{s.activity?.title || '—'}</td>
                        <td className="px-4 py-3 text-[12.5px] text-muted-foreground max-w-[150px] truncate">{s.activity?.location || '—'}</td>
                        <td className="px-4 py-3 text-[12.5px] text-muted-foreground whitespace-nowrap">
                          {s.checkin_time ? new Date(s.checkin_time).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}
                        </td>
                        <td className="px-4 py-3 text-[12.5px] text-muted-foreground font-tabular">
                          {s.checkin_time ? new Date(s.checkin_time).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' }) : '—'}
                        </td>
                        <td className="px-4 py-3 text-[12.5px] text-muted-foreground font-tabular">
                          {s.checkout_time ? new Date(s.checkout_time).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' }) : '—'}
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-[13px] font-700 text-primary font-tabular">{s.hours_spent || 0} hrs</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
