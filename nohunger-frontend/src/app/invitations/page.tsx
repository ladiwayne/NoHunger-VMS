      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-[14px] text-muted-foreground mt-0.5">
            Event invitations sent to you by the admin team
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-card border border-border rounded-2xl">
            ...
            <p className="text-[13px] text-muted-foreground mt-1">
              Invitations from the admin team will appear here
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map((inv) => (
              <div
                key={inv.id}
                className={`bg-card border rounded-2xl shadow-card p-5 ${inv.status === 'pending' ? 'border-warning/30' : 'border-border'}`}
              >
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className={`text-[11px] font-700 px-2.5 py-1 rounded-full border uppercase tracking-wide ${statusBadge(inv.status)}`}
                      >
                        {inv.status}
                      </span>
                    ...
                    <h3 className="text-[16px] font-700 text-foreground mb-1">
                    ...
                    {inv.status === 'accepted' && inv.activities?.checkin_link && (
                      <div className="mt-3 p-3 bg-primary/8 border border-primary/20 rounded-xl">
                        <p className="text-[12px] font-700 text-primary mb-1">
                          Check-in Link (venue only)
                        </p>
                        <p className="text-[11px] text-muted-foreground mb-2">
                          This link is only active at the event venue
                        </p>
                        <a
