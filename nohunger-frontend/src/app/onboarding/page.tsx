...
import {
  Heart, Package, Truck, ChefHat, ClipboardList, Users,
  MapPin, Phone, User, CheckCircle2, Clock, ArrowRight,
  ArrowLeft, Loader2, Star, Shield, Bell, Mail,
  Briefcase, Instagram, Twitter, Shirt, MessageSquare, Building2,
} from 'lucide-react';
...
const SKILLS = [
  { id: 'food-packing', label: 'Food Packing', icon: Package, desc: 'Sorting and packing food items' },
  { id: 'food-distribution', label: 'Distribution', icon: Truck, desc: 'Delivering food to communities' },
  ...
  { id: 'community-outreach', label: 'Community Outreach', icon: Users, desc: 'Engaging with communities' },
  { id: 'medical-support', label: 'Medical Support', icon: Heart, desc: 'Health and wellness support' },
];
...
const inputBase = 'w-full px-3 py-2.5 bg-muted border rounded-xl text-[14px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 transition-all';
const inputOk = 'border-border focus:ring-[hsl(142,72%,29%)]/25 focus:border-[hsl(142,72%,29%)]';
const inputErr = 'border-destructive/60 focus:ring-destructive/20 focus:border-destructive bg-destructive/5';
const inputWithIcon = 'pl-9';

const fieldCls = (hasErr: boolean) => `${inputBase} ${hasErr ? inputErr : inputOk}`;
const fieldIconCls = (hasErr: boolean) => `${inputBase} ${inputWithIcon} ${hasErr ? inputErr : inputOk}`;
...
export default function OnboardingPage() {
  ...
  const validateProfile = (): boolean => {
    ...
    if (anyBirthday) {
      ...
      if (!form.birthdayYYYY || form.birthdayYYYY.length !== 4 || yyyy < 1900 || yyyy > new Date().getFullYear()) e.birthdayYYYY = 'Invalid year';
    }
    setErrors(e);
  ...
  const toggleAvailability = (id: string) => {
    setSelectedAvailability(prev => prev.includes(id) ? prev.filter(a => a !== id) : [...prev, id]);
  };
            ...
            <div className="flex items-center gap-1 sm:gap-1.5">
              {(['welcome', 'profile', 'skills'] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-1 sm:gap-1.5">
                  <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[10px] sm:text-[11px] font-700 transition-colors ${
                    step === s ? 'bg-[hsl(142,72%,29%)] text-white' :
                    (['welcome', 'profile', 'skills'].indexOf(step) > i) ? 'bg-[hsl(142,72%,90%)] text-[hsl(142,72%,22%)]' :
                    'bg-muted text-muted-foreground'
                  }`}>
                    {(['welcome', 'profile', 'skills'].indexOf(step) > i) ? <CheckCircle2 size={12} /> : i + 1}
                  </div>
                  {i < 2 && <div className={`w-5 sm:w-8 h-0.5 rounded-full ${(['welcome', 'profile', 'skills'].indexOf(step) > i) ? 'bg-[hsl(142,72%,29%)]' : 'bg-border'}`} />}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="w-full max-w-3xl">
            ...
            <div className="text-center animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                {[
                  ...
                  return (
                    <div key={item.title} className="bg-card border border-border rounded-xl p-5 text-left">
                      ...
                      <p className="text-muted-foreground text-[12px] leading-relaxed">{item.desc}</p>
                    </div>
                  );
                })}
              </div>
          ...
          {step === 'profile' && (
            <div className="animate-fade-in">
              <div className="mb-6">
                <h2 className="text-xl sm:text-2xl font-800 text-foreground mb-1">Complete your profile</h2>
                <p className="text-muted-foreground text-[14px]">Help us know you better so we can connect you with the right activities.</p>
              </div>
              ...
              <div className="space-y-5">
                ...
                <div className="bg-card border border-border rounded-2xl p-4 sm:p-6">
                  ...
                  <div className="space-y-4">
                    {/* Email */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Email Address <span className="text-destructive">*</span></label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">First Name <span className="text-destructive">*</span></label>
                        <div className="relative">
                      ...
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Last Name <span className="text-destructive">*</span></label>
                        <input
                          type="text"
                          placeholder="Doe"
                          value={form.lastName}
                    ...
                    {/* Gender */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Gender <span className="text-destructive">*</span></label>
                      <select
                        value={form.gender}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Phone Number <span className="text-destructive">*</span></label>
                        <div className="relative">
                      ...
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Alternate Phone Number</label>
                        <div className="relative">
                          <Phone size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                          <input
                            type="tel"
                    ...
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">
                        Birthday <span className="text-muted-foreground font-400 text-[12px]">(optional)</span>
                      </label>
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <input
                            ...
                            onChange={e => setField('birthdayMM', e.target.value.replace(/\D/g, ''))}
                            className={`${fieldCls(!!errors.birthdayMM)} text-center`}
                          />
                          <p className="text-[11px] text-muted-foreground text-center mt-1">Month</p>
                        ...
                        <div>
                          <input
                            ...
                            onChange={e => setField('birthdayDD', e.target.value.replace(/\D/g, ''))}
                        ...
                        <div>
                          <input
                            ...
                            onChange={e => setField('birthdayYYYY', e.target.value.replace(/\D/g, ''))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Occupation</label>
                        <div className="relative">
                      ...
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Organization / Place of Work</label>
                        <div className="relative">
                    ...
                    {/* Shirt Size */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Shirt Size <span className="text-destructive">*</span></label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Instagram Handle</label>
                        <div className="relative">
                      ...
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">Twitter Handle</label>
                        <div className="relative">
                ...
                <div className="bg-card border border-border rounded-2xl p-4 sm:p-6">
                  ...
                  <div className="space-y-4">
                    {/* Street Address */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Street Address <span className="text-destructive">*</span></label>
                      <input
                        type="text"
                    ...
                    {/* Address Line 2 */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Address Line 2</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">City <span className="text-destructive">*</span></label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[13px] font-600 text-foreground mb-1.5">State <span className="text-destructive">*</span></label>
                        <select
                    ...
                    {/* Postal */}
                    <div>
                      <label className="block text-[13px] font-600 text-foreground mb-1.5">Postal / Zip Code</label>
                      <input
                        type="text"
          ...
          {step === 'skills' && (
            <div className="animate-fade-in">
              <div className="mb-8">
                <h2 className="text-xl sm:text-2xl font-800 text-foreground mb-1">Your skills & availability</h2>
                <p className="text-muted-foreground text-[14px]">Select the areas where you can contribute most. You can update these later.</p>
              </div>

              {/* Skills */}
              <div className="bg-card border border-border rounded-2xl p-4 sm:p-6 mb-4">
                <p className="text-[13px] font-700 text-foreground mb-3">Skill Areas <span className="text-destructive">*</span></p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {SKILLS.map(skill => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-2 ${selected ? 'bg-[hsl(142,72%,29%)]' : 'bg-border'}`}>
                          <SkillIcon size={15} className={selected ? 'text-white' : 'text-muted-foreground'} />
                        </div>
                        <p className={`text-[12px] font-700 ${selected ? 'text-[hsl(142,72%,22%)]' : 'text-foreground'}`}>{skill.label}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5 leading-tight hidden sm:block">{skill.desc}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="bg-card border border-border rounded-2xl p-4 sm:p-6 mb-6">
                <p className="text-[13px] font-700 text-foreground mb-3">Availability <span className="text-muted-foreground font-400">(optional)</span></p>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {saving ? <><Loader2 size={15} className="animate-spin" /> Submitting…</> : <>Submit Profile <ArrowRight size={15} /></>}
                </button>
              </div>
            </div>
          )}
