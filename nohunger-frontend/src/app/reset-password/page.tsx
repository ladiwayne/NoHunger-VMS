...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data: ResetFormData) => {
    ...
    try {
      ...
      setError('Password reset via email token is not configured in MongoDB mode yet. Please contact an administrator to reset your password.');
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6">
        <div className="text-center max-w-sm">
          ...
          <p className="text-[14px] text-muted-foreground">Your password has been changed successfully. Redirecting to sign in…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-2xl p-8 shadow-card">
          <div className="mb-6">
            ...
            <p className="text-[14px] text-muted-foreground mt-1">Choose a strong password for your account</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            <div>
              <label className="block text-[13px] font-600 text-foreground mb-1.5">New password</label>
              <div className="relative">
                <Lock size={16} className={`absolute left-3.5 top-1/2 -translate-y-1/2 ${errors.password ? 'text-destructive' : 'text-[hsl(142,72%,35%)]'}`} />
                <input
                  {...register('password', {
            ...
            <div>
              <label className="block text-[13px] font-600 text-foreground mb-1.5">Confirm new password</label>
              <div className="relative">
                <Lock size={16} className={`absolute left-3.5 top-1/2 -translate-y-1/2 ${errors.confirmPassword ? 'text-destructive' : 'text-[hsl(142,72%,35%)]'}`} />
                <input
                ...
                />
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-[hsl(142,72%,29%)] transition-colors">
            ...
            >
              {loading ? <Loader2 size={18} className="animate-spin" /> : <><span>Update Password</span><ArrowRight size={16} /></>}
            </button>
          </form>
        </div>
      </div>
