...
import { Clock, CalendarCheck, Bell, AlertTriangle, CheckCircle2, XCircle, Loader2, MapPin, Calendar, KeyRound, ArrowRight } from 'lucide-react';
...
export default function VolunteerDashboardPage() {
  ...
  const [stats, setStats] = useState({ totalHours: 0, eventsAttended: 0, pendingInvitations: 0, activeTasks: 0 });
  ...
  useEffect(() => {
    ...
    if (activeCheckin?.checkin_time) {
      interval = setInterval(() => {
        const elapsed = Math.floor((Date.now() - new Date(activeCheckin.checkin_time).getTime()) / 1000);
        setTimer(elapsed);
      }, 1000);
    }
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const upcoming = activities
        .filter(a => ['published', 'ongoing'].includes(a.status) && a.start_date && new Date(a.start_date) >= now)
      ...
      const last7Days = Array.from({ length: 7 }, (_, i) => {
        ...
        return { date: d.toLocaleDateString('en', { weekday: 'short' }), hours: 0, fullDate: d.toDateString() };
      });
  ...
  const handleCheckinCode = () => {
    ...
    if (!code) { toast.error('Please enter a check-in code.'); return; }
    ...
    <AppLayout activePath="/volunteer-dashboard">
      <div className="space-y-6 animate-fade-in">
        ...
        {dataLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link href="/hours-tracking" className="col-span-2 group bg-card border border-border rounded-2xl p-5 shadow-card hover:shadow-[0_4px_16px_0_rgba(22,101,52,0.12)] hover:border-[hsl(142,72%,78%)] transition-all">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="text-4xl font-700 font-tabular text-foreground">{stats.totalHours}</span>
                <span className="text-[13px] font-500 text-muted-foreground">hrs</span>
              </div>
              <p className="text-[12.5px] font-600 text-muted-foreground mt-0.5 uppercase tracking-wide">Total Hours Logged</p>
            </Link>
            <div className="bg-card border border-border rounded-2xl p-5 shadow-card hover:shadow-[0_4px_16px_0_rgba(22,101,52,0.10)] hover:border-[hsl(142,72%,78%)] transition-all">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl font-700 font-tabular text-foreground">{stats.eventsAttended}</span>
                <span className="text-[13px] font-500 text-muted-foreground">events</span>
              </div>
              <p className="text-[12.5px] font-600 text-muted-foreground mt-0.5 uppercase tracking-wide">Events Attended</p>
            </div>
            <Link href="/invitations" className={`group bg-card rounded-2xl p-5 shadow-card hover:shadow-[0_4px_16px_0_rgba(22,101,52,0.10)] transition-all ${stats.pendingInvitations > 0 ? 'border border-warning/30 bg-warning/4' : 'border border-border hover:border-[hsl(142,72%,78%)]'}`}>
              <div className="w-9 h-9 rounded-xl bg-warning/10 flex items-center justify-center mb-4">
                <Bell size={18} className="text-warning" />
              </div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl font-700 font-tabular text-foreground">{stats.pendingInvitations}</span>
                <span className="text-[13px] font-500 text-muted-foreground">pending</span>
              </div>
              <p className="text-[12.5px] font-600 text-muted-foreground mt-0.5 uppercase tracking-wide">Invitations</p>
            </Link>
          </div>
        )}
        ...
        <div className="bg-card border border-border rounded-2xl shadow-card p-5">
          ...
          <p className="text-[13px] text-muted-foreground mb-3">Enter the check-in code provided at your event to log your attendance.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-2xl shadow-card p-5">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={weeklyData}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: 'hsl(140,10%,48%)' }} axisLine={false} tickLine={false} />
                ...
                <Area type="monotone" dataKey="hours" stroke="hsl(142,72%,29%)" strokeWidth={2} fill="url(#hoursGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/activities" className="text-[12px] text-primary font-600 hover:underline">View all</Link>
            </div>
            {dataLoading ? (
              <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-14 bg-muted rounded-xl animate-pulse" />)}</div>
            ) : upcomingEvents.length === 0 ? (
              <p className="text-[13px] text-muted-foreground text-center py-6">No upcoming events</p>
            ) : (
                ...
                {upcomingEvents.map(event => (
                  <div key={event.id} className="p-3 bg-muted rounded-xl">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <p className="text-[11px] text-muted-foreground">{new Date(event.start_date).toLocaleDateString('en', { month: 'short', day: 'numeric' })}</p>
                    </div>
                    {event.location && (
          ...
          <div className="bg-card border border-border rounded-2xl shadow-card p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground">{invitations.length} invitation{invitations.length > 1 ? 's' : ''} awaiting your response</p>
              </div>
              <Link href="/invitations" className="text-[12px] text-primary font-600 hover:underline">View all</Link>
            </div>
            <div className="space-y-3">
              {invitations.slice(0, 3).map(inv => (
                <div key={inv.id} className="flex items-center justify-between gap-4 p-4 bg-warning/5 border border-warning/20 rounded-xl">
                  <div className="flex-1 min-w-0">
                    <p className="text-[13.5px] font-600 text-foreground truncate">{inv.activities?.title}</p>
                    <p className="text-[12px] text-muted-foreground mt-0.5">
                      {inv.activities?.start_date ? new Date(inv.activities.start_date).toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' }) : ''}
                      {inv.activities?.location ? ` · ${inv.activities.location}` : ''}
                    </p>
                    {inv.message && <p className="text-[11px] text-muted-foreground mt-1 italic truncate">"{inv.message}"</p>}
                  </div>
